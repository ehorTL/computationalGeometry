...
import beziersurface.drawer.RotationGroup;
...
public class Drawer3D extends Application {
    ...
    public void start(Stage stage) throws Exception {
        ...
        for (Point3D[] pa : bs.getControlPoints()){
            for (Point3D p : pa){
                p = p.multiply(SCALE);
                controlPoints.add(new Point(p.getX(), p.getY(), p.getZ(), Color.BLUE));
            }
        }
        ArrayList<Point3D> pointsGenerated = bs.generateSurfacePoints();
        ArrayList<Point> pointsGeneratedCustomized = new ArrayList<>();
        for (Point3D p : pointsGenerated){
            p = p.multiply(SCALE);
            pointsGeneratedCustomized.add(new Point(p.getX(), p.getY(), p.getZ(), Color.SILVER));
        }
        root.getChildren().addAll(pointsGeneratedCustomized);
        root.getChildren().addAll(controlPoints);
